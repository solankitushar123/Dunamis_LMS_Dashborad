  ...
  const feedbackExamples = {
    "keyboard-fundamentals": [
      ...
      },
      {
        date: "15 May 2025",
        feedback:
          "You made progress on both homework and practice. Focus on clean transitions between chords.",
      },
    ],
    "spanish-for-business": [
      ...
      },
      {
        date: "15 May 2025",
        feedback:
          "Listening skills have improved. Practice spoken grammar to match writing proficiency.",
      },
    ],
...
export default function MyPerformancePage() {
  ...
  return (
    <main
      className="w-full px-6 lg:px-14 py-7 min-h-screen bg-white"
      style={{ fontFamily: "'Inter', sans-serif" }}
    >
      {/* Breadcrumb */}
      <nav className="mb-6 flex items-center text-sm text-[#6b7280] gap-2">
      ...
      <div className="flex flex-col lg:flex-row gap-14">
        ...
        <div className="flex-1">
          <h3 className="text-[#111827] font-semibold text-lg mb-1">
            {selectedCourse ? selectedCourse.title : courses?.title}
          </h3>
          <p className="text-[#6b7280] text-sm mb-4">
            Check your monthly stats
          </p>
          <div className="rounded-2xl bg-white px-8 py-6 shadow border border-[#f3f4f6]">
        ...
        <div className="w-full lg:max-w-sm">
          ...
          <button className="border border-[#e5e7eb] rounded-lg py-2 px-5 bg-white hover:bg-gray-50 transition text-sm font-medium text-[#111827]">
            View All
          </button>
        </div>
      </div>
...
function CourseCard({ course, selected, onClick, wide }) {
  return (
    <div
      onClick={onClick}
      className={`flex cursor-pointer bg-white rounded-2xl border overflow-hidden shadow-sm hover:shadow-md transition h-[114px] ${
        selected === course.id
    ...
    >
      <div
        className={`${wide ? "w-60" : "w-44 sm:w-56"} h-full overflow-hidden`}
      >
        <img
          src={course.image}
          alt={course.title}
      ...
      <div className="flex-1 px-4 py-3 flex flex-col justify-center">
        <div className="flex flex-wrap mb-1">
          {course.tags.map((tag, i) => (
            <span
              key={i}
              className="rounded px-2 py-0.5 mr-2 mb-1"
              style={{
                background: tag.bg,
                color: tag.color,
                fontWeight: 500,
        ...
        <div className="leading-tight mb-1 text-[#111827] font-semibold text-base">
          {course.title}
        </div>
        <div className="text-[#6b7280] text-sm">by {course.instructor}</div>
      </div>
    </div>
  );
}
        ...
        options={{
          ...
          scales: {
            x: {
              ...
              grid: { display: false },
              ticks: {
                font: { family: "Inter", weight: "400", size: 14 },
                color: "#111827",
                padding: 4,
              },
            },
